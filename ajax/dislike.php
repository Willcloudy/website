<?php 
    ...
    $delete_like = "DELETE from `like_post` WHERE `post_id` = '$post_id' and `user_id` = '$user_id'";
    $deleted = mysqli_query($con, $delete_like);

    if ($deleted) {
        $like_post = "UPDATE `posts` SET `post_like` = '$update_like' WHERE post_id = '$post_id'";
        $update_query = mysqli_query($con, $like_post);

        $update_affect = mysqli_affected_rows($con)>0;
        if ($deleted and $update_query and $update_affect) {
            echo "
            <ul class='article-function'>
                ...
                <li>
                    <a>
                        <button class='btn btn-outline-success' onclick='shoucang(this)' >
